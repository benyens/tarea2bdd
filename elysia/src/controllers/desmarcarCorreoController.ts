...
export async function desmarcarCorreoController(body: { usuarioId: string; clave: string; correoId: number }): Promise<any> {
    ...
    try {
        ...
        const correoFavorito = await prisma.correos_favoritos.findUnique({
            where: { id: correoId },
        });
    
        if (correoFavorito?.es_favorito === false) {
